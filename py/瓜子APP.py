...
class Spider(BaseSpider):
    ...
    def categoryContent(self, tid, pg, filter, extend):
        ...
        try:
            body = {
                "area": extend.get('area', '0') if extend else '0',
                "year": extend.get('year', '0') if extend else '0',
                "pageSize": "30",
                "sort": extend.get('sort', 'd_id') if extend else 'd_id',
                "page": str(pg),
                "tid": tid
            }
    ...
    def detailContent(self, array):
        try:
            vod_id = array[0].split('/')[0]
            t = str(int(time.time()))
            body1 = {"token_id": "1649412", "vod_id": vod_id, "mobile_time": t, "token": self.token}
    ...
    def searchContent(self, key, quick, pg=1):
        ...
        try:
            ...
            if data and 'list' in data:
                ...
                for item in data['list']:
                    vod_name = item.get('vod_name', '')
                    if key_lower in vod_name.lower():
                        vod_continu = item.get('vod_continu', 0)
                        remarks = '电影' if vod_continu == 0 else f'更新至{vod_continu}集'
                        video = {
                            "vod_id": f"{item.get('vod_id', '')}/{vod_continu}",
        ...
        try:
            ...
            if resolutions:
                ...
                if data and 'url' in data:
                    return {"parse": 0, "url": data['url'], "header": {
    ...
    def get_data(self, data, path, use_cache=True):
        try:
            ...
            body = {
                'token': self.token, 'token_id': '', 'phone_type': '1', 'time': t,
                'phone_model': 'xiaomi-22021211rc', 'keys': keys, 'request_key': request_key,
                'signature': signature, 'app_id': '1', 'ad_version': '1'
            }
            url = f"{self.host}{path}"
            response = self.session.post(url, headers=self.header, data=body, timeout=10)
